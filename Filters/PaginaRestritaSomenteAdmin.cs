...
namespace ControleDeContatos.Filters
{
     ...
     * o usuário para a página de login, impedindo o acesso não autorizado. */

    public class PaginaRestritaSomenteAdmin : ActionFilterAttribute  // São os filtros do ASP.NET MVC.
    {
        public override void OnActionExecuting(ActionExecutingContext context) // Estou sobrescrevendo o método "OnActionExecuting", do "ActionFilterAttribute".
        {
            ...
            else
            {
                ...
                {
                    context.Result = new RedirectToRouteResult(new RouteValueDictionary { { "controller", "Restrito" }, { "action", "Index" } }); // Se o perfil do usuário não for de administrador, redireciona ele para uma rota restrita.
